...
// Your project imports (keep them, comment if not available while testing)
import 'package:snap2bill/Distributordirectory/home_page.dart';
import 'package:snap2bill/distributor_registration.dart';
import 'package:snap2bill/customer_registration.dart';
import 'Customerdirectory/customer_home_page.dart';

// ---------- DESIGN CONSTANTS (kept short) ----------
class AppColors {
  static const Color primaryLight = Color(0xFF4A69FF);
  static const Color backgroundLight = Color(0xFFF0F4F8);
  static const Color cardLight = Colors.white;
...
// ---------- MAIN (login_page) WITH THEME PERSISTENCE ----------
class login_page extends StatefulWidget {
  const login_page({Key? key}) : super(key: key);
...
class _login_pageState extends State<login_page> {
  ...
  Future<void> _loadTheme() async {
    ...
    final saved = sh.getBool('isDarkMode') ?? false;
    setState(() {
      _isDark = saved;
      _loaded = true;
    });
  ...
  Future<void> _toggleTheme() async {
    ...
    await sh.setBool('isDarkMode', _isDark);
  }

  @override
  Widget build(BuildContext context) {
    // While loading SharedPreferences, show a blank container to avoid flashing
...
// ---------- SUB WIDGET (login UI + logic) ----------
class login_page_sub extends StatefulWidget {
  final VoidCallback? onThemeChanged;
  final bool isDarkMode;

  const login_page_sub({Key? key, this.onThemeChanged, this.isDarkMode = false})
      : super(key: key);
...
class _login_page_subState extends State<login_page_sub>
  ...
  TextEditingController(text: "ashoka@gmail.com");
  TextEditingController password = TextEditingController(text: "password");
  bool _obscureText = true;
  ...
  Future<void> _login() async {
    ...
    SharedPreferences sh = await SharedPreferences.getInstance();
    String ip = sh.getString("ip") ?? "http://127.0.0.1:8000";
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // Theme toggle button (saves using SharedPreferences in parent)
          Positioned(
            top: 50,
            right: 20,
            child: InkWell(
  ...
  Widget _buildInputField({
    ...
    return Column(
      ...
      children: [
        Text(label,
            style:
            TextStyle(color: hintColor, fontWeight: FontWeight.w500, fontSize: 13)),
        const SizedBox(height: 8),
        Container(
          decoration: BoxDecoration(
            color: fillColor,
