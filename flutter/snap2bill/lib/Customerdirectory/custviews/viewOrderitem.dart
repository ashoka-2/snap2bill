...
class _ViewOrderItemsState extends State<ViewOrderItems> {
  ...
  // 1. Checks if the order is already processed to lock editing
  void _checkPaymentStatus() async {
    SharedPreferences sp = await SharedPreferences.getInstance();
    String status = sp.getString("order_payment_status") ?? "pending";
    setState(() {
      // Payment or processing done: lock the UI
  ...
  // 2. Updated to include the 'role' parameter for your backend logic
  Future<void> updateItem(String itemId, String qty) async {
    ...
    final res = await http.post(
      ...
      body: {
        'id': itemId,
        'quantity': qty,
        'role': 'customer' // Identify as customer to backend
      },
    );
    ...
    if (js['status'] == 'ok') {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(js['message'] ?? "Error updating item"))
      );
    }
  }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                // 3. Hide edit/delete if payment is confirmed
                if (!isPaid) ...[
                  const Divider(height: 24),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.end,
            ...
            AppButton(
              ...
              onPressed: () {
                ...
                if (qty == null || qty <= 0 || qty > 100) {
                  ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text("Enter a valid quantity (1-100)"))
                  );
                } else {
