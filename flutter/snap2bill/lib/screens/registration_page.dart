...
class _RegistrationPageState extends State<RegistrationPage> {
  ...
  Future<void> _submitRegistrationData() async {
    ...
    try {
      ...
      if (decoded['status'] == 'ok') {
        CustomSnackBar.show(context, "Registration Successful!", backgroundColor: AppColors.getSuccessColor(context));
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const LoginPage()));
      } else {
        _showError("Registration failed: ${decoded['message'] ?? decoded['status']}");
      }
    } catch (e) {
      _showError("Connection Error: Make sure IP is correct and server is running.");
    } finally {
      if (mounted) setState(() => _isLoading = false);
    }
  }

  // 🚀 NAYA REGISTER LOGIC (OTP SHIELD KE SATH)
  Future<void> _register() async {
    ...
    // 1. Agar Google se link kiya hai toh Email pehle se verified hai, Seedha Register karo!
    if (_isGoogleLinked) {
      await _submitRegistrationData();
      return;
    }

    // 2. Agar Manual type kiya hai, toh pehle OTP bhejo
    setState(() => _isLoading = true);
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      ...
      final response = await http.post(
          Uri.parse('$ip/send_verification_otp'),
      ...
      if (data['status'] == 'ok') {
        String realOtp = data['otp'].toString();
        _showOtpDialog(realOtp); // OTP Dialog Dikhai dega
      } else {
        ...
        builder: (context) {
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text("Enter the 6-digit OTP sent to\n${email.text}", textAlign: TextAlign.center),
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () {
                  if (otpController.text.trim() == realOtp) {
                    Navigator.pop(context); // Dialog band karo
                    _submitRegistrationData(); // Data Database me save karo!
                  } else {
                    _showError("Incorrect OTP! Try again.");
                  }
                },
                style: ElevatedButton.styleFrom(backgroundColor: AppColors.getPrimaryColor(context)),
