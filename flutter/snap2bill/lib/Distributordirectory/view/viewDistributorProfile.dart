...
class _ViewDistributorProfileState extends State<ViewDistributorProfile> {
  ...
  // ✅ LOGIC FIXED: Check cid to identify Customer
  ...
  Future<void> _fetchAllData() async {
    ...
    // ✅ FIX: According to your login page, cid exists for customers
    String? cid = sh.getString("cid");
    setState(() {
      _isCustomer = (cid != null && cid.isNotEmpty);
    });
  ...
  Future<void> _getProfile() async {
    try {
      ...
      if (jsonData["data"] != null && jsonData["data"].isNotEmpty) {
        var item = jsonData["data"][0];
        String imgUrl = (item["profile_image"] != null && item["profile_image"] != "null")
            ? "$ip${item["profile_image"]}" : "";

        setState(() {
          _profile = DistributorProfileModel(
            item["id"].toString(), item["name"].toString(), item["email"].toString(),
            item["phone"].toString(), imgUrl, item["bio"].toString(),
            item["address"].toString(), item["place"].toString(), item["pincode"].toString(),
            item["post"].toString(), item["latitude"].toString(), item["longitude"].toString(), "",
          );
        });
      }
  ...
  Future<void> _makeCall() async {
    if (_profile == null) return;
    await launchUrl(Uri(scheme: 'tel', path: _profile!.phone));
  }

  Future<void> _messageDistributor() async {
    if (_profile == null) return;
    final Uri whatsappUri = Uri.parse("whatsapp://send?phone=+91${_profile!.phone}");
    if (await canLaunchUrl(whatsappUri)) {
      await launchUrl(whatsappUri);
    } else {
      await launchUrl(Uri(scheme: 'sms', path: _profile!.phone));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _isLoading ? _buildPageShimmer(theme) : RefreshIndicator(
        onRefresh: _fetchAllData,
        child: CustomScrollView(
          physics: const BouncingScrollPhysics(),
          slivers: [
            SliverToBoxAdapter(child: _buildProfileHeader(theme, textColor, subTextColor)),
            SliverToBoxAdapter(child: _buildToggleTabs(isDark, iconColor, theme)),
            if (_showDetails) SliverToBoxAdapter(child: _buildContactDetails(theme, textColor, subTextColor))
            else _products.isEmpty ? _buildEmptyProducts(subTextColor) : _buildProductGrid(theme),
            const SliverPadding(padding: EdgeInsets.only(bottom: 50)),
          ],
        ),
      ),
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              CircleAvatar(
                radius: 40, backgroundColor: theme.cardColor,
                backgroundImage: _profile!.profile_image.isNotEmpty ? NetworkImage(_profile!.profile_image) : null,
                child: _profile!.profile_image.isEmpty ? Icon(Icons.person, size: 40, color: subTextColor) : null,
              ),
              const SizedBox(width: 20),
              Expanded(
                child: Row(
          ...
          if (_profile!.bio.isNotEmpty && _profile!.bio != "null") Text(_profile!.bio, style: TextStyle(color: subTextColor)),
          Row(children: [Icon(Icons.location_on, size: 14, color: theme.primaryColor), Text(" ${_profile!.place}", style: TextStyle(color: theme.primaryColor))]),
          const SizedBox(height: 20),
          Row(
            children: [
              Expanded(
                child: ElevatedButton.icon(
                  onPressed: _makeCall, icon: const Icon(Icons.call, size: 18), label: const Text("Call"),
                  style: ElevatedButton.styleFrom(backgroundColor: theme.primaryColor, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
                ),
              ),
              const SizedBox(width: 10),
              Expanded(
                child: ElevatedButton.icon(
                  onPressed: _messageDistributor, icon: const Icon(Icons.message, size: 18), label: const Text("Message"),
                  style: ElevatedButton.styleFrom(backgroundColor: Colors.blueGrey, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
                ),
              ),
            ],
          ),
  ...
  Widget _buildToggleTabs(bool isDark, Color iconColor, ThemeData theme) {
    return Column(children: [
      Divider(height: 1, color: isDark ? Colors.grey.shade800 : Colors.grey.shade300),
      Row(children: [
        Expanded(child: InkWell(onTap: () => setState(() => _showDetails = false), child: Padding(padding: EdgeInsets.all(15), child: Icon(Icons.grid_on, color: !_showDetails ? theme.primaryColor : iconColor)))),
        Expanded(child: InkWell(onTap: () => setState(() => _showDetails = true), child: Padding(padding: EdgeInsets.all(15), child: Icon(Icons.info_outline, color: _showDetails ? theme.primaryColor : iconColor)))),
      ]),
      Divider(height: 1, color: isDark ? Colors.grey.shade800 : Colors.grey.shade300),
    ]);
  }
          ...
          child: Stack(
            children: [
              Hero(
                ...
                child: Container(
                  decoration: BoxDecoration(border: Border.all(color: Colors.grey.withOpacity(0.1), width: 0.5)),
                  child: Image.network(product.image, fit: BoxFit.cover, errorBuilder: (c, e, s) => Icon(Icons.broken_image)),
                ),
              ),
              // ✅ ADD TO CART OVERLAY: Based on _isCustomer
              if (_isCustomer)
                Positioned(
                  bottom: 5,
                  right: 5,
                  child: Container(
                    ...
                    decoration: BoxDecoration(color: theme.primaryColor.withOpacity(0.9), shape: BoxShape.circle),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildDetailRow(Icons.pin_drop_outlined, "Pincode", _profile!.pincode, textColor, subTextColor),
          const SizedBox(height: 30),
          SizedBox(width: double.infinity, height: 50, child: ElevatedButton.icon(onPressed: _launchMaps, icon: const Icon(Icons.map, color: Colors.white), label: const Text("Open Location in Maps", style: TextStyle(color: Colors.white)), style: ElevatedButton.styleFrom(backgroundColor: theme.primaryColor, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))))),
        ],
      ),
    );
  }
  ...
  Widget _buildDetailRow(IconData icon, String label, String value, Color textColor, Color subTextColor) {
    return Padding(
      ...
      child: Row(children: [
        Icon(icon, size: 20, color: subTextColor),
        const SizedBox(width: 15),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: TextStyle(fontSize: 12, color: subTextColor)), Text(value.isEmpty ? "Not set" : value, style: TextStyle(fontSize: 15, color: textColor, fontWeight: FontWeight.w500))])),
      ]),
    );
  }

  Widget _buildStatColumn(String count, String label, Color textColor, Color subTextColor) {
    return Column(children: [Text(count, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: textColor)), Text(label, style: TextStyle(color: subTextColor, fontSize: 13))]);
  }

  void _showProductDetails(ProductModel product, ThemeData theme) {
    showModalBottomSheet(
      context: context, isScrollControlled: true, backgroundColor: theme.cardColor,
      shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))),
      builder: (context) => DraggableScrollableSheet(
        expand: false, initialChildSize: 0.6, maxChildSize: 0.9,
        builder: (context, sc) => ListView(controller: sc, padding: EdgeInsets.all(20), children: [
          ClipRRect(borderRadius: BorderRadius.circular(15), child: Image.network(product.image, height: 300, fit: BoxFit.contain)),
          const SizedBox(height: 20),
          Text(product.product_name, style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
          Text("₹ ${product.price}", style: TextStyle(fontSize: 20, color: theme.primaryColor, fontWeight: FontWeight.bold)),
          const SizedBox(height: 15),
          Text("Description", style: TextStyle(fontWeight: FontWeight.bold)),
          Text(product.description, style: TextStyle(color: Colors.grey, height: 1.5)),
        ]),
      ),
    );
  }

  Widget _buildEmptyProducts(Color subTextColor) {
    return SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.only(top: 80.0), child: Column(children: [Icon(Icons.inventory_2_outlined, size: 60, color: subTextColor), const SizedBox(height: 10), Text("No products listed", style: TextStyle(color: subTextColor))])));
  }

  Widget _buildPageShimmer(ThemeData theme) {
    final isDark = theme.brightness == Brightness.dark;
    return Shimmer.fromColors(
      baseColor: isDark ? Colors.grey[800]! : Colors.grey[300]! , highlightColor: isDark ? Colors.grey[700]! : Colors.grey[100]!,
      child: SingleChildScrollView(child: Padding(padding: const EdgeInsets.all(16.0), child: Column(children: [Row(children: [CircleAvatar(radius: 40), SizedBox(width: 20), Expanded(child: Container(height: 10, color: Colors.white))])]))),
    );
  }
}
