...
class _Home_pageState extends State<Home_page> {
  ...
  Future<void> _loadData() async {
    ...
    // Fetch products sorted by Latest (via backend)
  ...
  List<DrawerItemModel> _getDrawerItems() {
    return [
      DrawerItemModel(icon: Icons.people_alt_outlined, title: "Distributors", onTap: () => const view_distributors()),
      DrawerItemModel(icon: Icons.inventory_2_outlined, title: "My Products", onTap: () => const myProducts()),
      DrawerItemModel(icon: Icons.shopping_bag_outlined, title: "All Products", onTap: () => const view_product()),
      DrawerItemModel(icon: Icons.list_alt, title: "Orders", onTap: () => const viewOrder()),
      DrawerItemModel(icon: Icons.category_outlined, title: "Category", onTap: () => const view_category()),
      DrawerItemModel(icon: Icons.feedback_outlined, title: "Feedback", onTap: () => const view_feedback()),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Snap2Bill", style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
        leading: IconButton(
          ...
          IconButton(
            onPressed: () {
              Navigator.push(context, MaterialPageRoute(builder: (_) => const ViewWishlist()))
                  .then((_) => _loadData()); // Reload to sync liked icons
            },
